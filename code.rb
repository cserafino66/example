...
twitter_data.each do |handle|
  print handle.keys[0] + ": "
  handle.each do |key, value|
    if value["description"] == nil
      puts "NA"
    else puts value["description"]
    end
  end
end
...
twitter_data.each do |handle|
  print handle.keys[0]
  handle.each do |key, value|
    puts "'s last tweet was #{value["latest tweet"].length} characters long."
  end
end
...
twitter_data.each do |handle|
  ...
  handle.each do |key, value|
    value["last twenty tweets"].each do |tweet|
      character_sum += tweet.length
    end
    puts " used #{character_sum} characters in their last 20 tweets."
    character_sum = 0
  end
end
...
puts "#{most_followed_user} is the most followed handle with #{max_followers} followers."
...
twitter_data.each do |handle|
  handle.each do |key, value|
    if value["number of friends"] > max_friends
      ...
      most_friended_user = key
    end
  end
end
puts "#{most_friended_user} has the most friends with #{max_friends} friends."
...
puts "#{tweets_the_most} has the most tweets with #{max_tweets} tweets."


puts "===="
# * users w/description in twitter_data

users_with_description = []
twitter_data.each do |handle|
  handle.each do |key, value|
    if value["description"] != nil
      users_with_description << key
    end
  end
end
puts "These are the users with descriptions:"
users_with_description.each do |name|
  puts name
end

puts "===="
# * users w/location in twitter_data

users_with_location = []
twitter_data.each do |handle|
  handle.each do |key, value|
    if value["location"] != nil
      users_with_location << key
    end
  end
end
puts "These are the users with locations:"
